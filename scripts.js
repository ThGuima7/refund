...
function updateTotals() {
    try {
        ...
        expenseQuantity.textContent = `${items.length} ${items.length > 1 ? "despesas" : "despesa"}` // corrigido length
        ...
        for (let i = 0; i < items.length; i++) { 
            ...
            const value = itemAmount.textContent
                .replace(/[^\d,]/g, "")
                .replace(",", ".")
            ...
            if (isNaN(valueFormatted)) {
                alert("Não foi possível calcular total")
                return
            }
